...
def da_prep_4_freyberg_batch():
    import flopy
    org_ws = os.path.join("da_monthly_freyberg_test","temp_monthly")
    batch_ws = os.path.join("da_monthly_freyberg_test","template_batch")
    if os.path.exists(batch_ws):
        shutil.rmtree(batch_ws)
    shutil.copytree(org_ws,batch_ws)
    sim = flopy.mf6.MFSimulation.load(sim_ws=org_ws)
    m = sim.get_model("freyberg6")
    sr = m.modelgrid
    # set up PstFrom object
    pf = pyemu.prototypes.PstFrom(original_d=org_ws, new_d=batch_ws,
    ...
    df = pd.read_csv(os.path.join(batch_ws, "sfr.csv"), index_col=0)
    ...
    for tag,bnd in tags.items():
        lb,ub = bnd[0],bnd[1]
        arr_files = [f for f in os.listdir(batch_ws) if tag in f and f.endswith(".txt")]
        if len(arr_files) == 0:
            raise Exception("no files for tag: "+tag)
        if "rch" in tag:
...
def da_prep_4_freyberg_seq():
    import flopy
    org_ws = os.path.join("da_monthly_freyberg_test","temp_seq")
    seq_ws = os.path.join("da_monthly_freyberg_test","template_seq")
    if os.path.exists(seq_ws):
        shutil.rmtree(seq_ws)

    sim = flopy.mf6.MFSimulation.load(sim_ws=org_ws)
    m = sim.get_model("freyberg6")
    sr = m.modelgrid
    # set up PstFrom object
    pf = pyemu.prototypes.PstFrom(original_d=org_ws, new_d=seq_ws,
                 remove_existing=True,
                 longnames=True, spatial_reference=sr,
                 zero_based=False)
    # obs
    ...
    df = pd.read_csv(os.path.join(seq_ws, "sfr.csv"), index_col=0)
    pf.add_observations("sfr.csv", insfile="sfr.csv.ins", index_cols="time", use_cols=list(df.columns.values))
    v = pyemu.geostats.ExpVario(contribution=1.0,a=1000)
    gr_gs = pyemu.geostats.GeoStruct(variograms=v)
    pf.extra_py_imports.append('flopy')
    ib = m.dis.idomain[0].array
    tags = {"npf_k_":[0.1,10.],"npf_k33_":[.1,10],"sto_ss":[.1,10],"sto_sy":[.9,1.1],"rch_recharge":[.5,1.5],"ic_strt":[0.9,1.1]}
    for tag,bnd in tags.items():
        lb,ub = bnd[0],bnd[1]
        arr_files = [f for f in os.listdir(seq_ws) if tag in f and f.endswith(".txt")]
        if len(arr_files) == 0:
            raise Exception("no files for tag: "+tag)
        if "rch" in tag:
            pf.add_parameters(filenames=arr_files, par_type="grid", par_name_base="rch_gr",
    ...
    list_file = ["freyberg6.wel_stress_period_data_{0}.txt".format(t)
                  for t in range(1,m.nper+1)][0]
    # add temporally indep, but spatially correlated wel flux pars
    pf.add_parameters(filenames=[list_file], par_type="grid", par_name_base="wel_grid",
                      pargp="wel_grid", index_cols=[0, 1, 2], use_cols=[3],
                      upper_bound=1.5, lower_bound=0.5, geostruct=gr_gs,mfile_skip=1)
    ...
    df = pd.read_csv(os.path.join(seq_ws, "heads.csv"), index_col=0)
    pf.add_observations("heads.csv", insfile="heads.csv.ins", index_cols="time", use_cols=list(df.columns.values),
                        prefix="hds", rebuild_pst=True)
