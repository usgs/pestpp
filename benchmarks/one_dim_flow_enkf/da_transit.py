...
We write a Python warapper that takes one file for each parameter group and generate one output file as output. Let us look at the model input files. Output file is similair to observation data section. This is just a personal prefernce! you may still deals with the native MODFLOW I/O files.
* input_dis.csv
* hk_1.dat
* iheads_1.dat
"""
...
# %%
"""
In sequential Data Assimilation, we need to be able to rerun the model starting from any arbitrary time point and stop at any other time point . The Python wraper, that works as our model, uses a simple csv input file with two parameters: start time and end time. In this example we are going to divide the 24 stress periods into 4 cycles. 
"""
...
# %%
"""
## Generate prior paramaters/states ensemble
...
"""
# Observations
As mesnioned before, DA combine model with observations. This is just a synthatic problem, so we need to generate synthatic observations. The observation is generated by simulating a truth parameters (refernce realization), which is a a user defined in the init file.
...
# %%
# (4) Assign each observation to a cycle number. In this case study we devide the 24 stress periods into 4 cycles, each has 6 stress periods.
...
pst.observation_data['obsval'] = 0.0 # let put 0.0 here becuase observation will be read each cycle fro the cycle table
