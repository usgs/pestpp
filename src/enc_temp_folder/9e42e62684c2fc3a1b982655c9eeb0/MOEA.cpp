...
map<string, double> ParetoObjectives::get_cluster_crowding_fitness(vector<string>& members, map<string, map<string, double>>& _member_struct)
{
	...
	for (auto obj_map : obj_member_map)
	{
		...
		for (auto m : members)
		{
			if (_member_struct[m][obj_map.first + "_SD"] == 0)
				count_true++;
		}
		...
		//else
		min_sd[obj_map.first] = 10 * (last->second - start->second) / (pest_scenario.get_pestpp_options().get_mou_max_archive_size());
