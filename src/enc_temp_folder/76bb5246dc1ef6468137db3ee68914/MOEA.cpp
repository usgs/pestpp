...
map<string, double> ParetoObjectives::get_cluster_crowding_fitness(vector<string>& members, map<string, map<string, double>>& _member_struct)
{
	...
	for (auto m : members)
	{
		map<string, double> nn_dist = expdist_lookup[m];
		...
		for (; inextnn != nn_sorted.end(); ++inextnn)
		{
			...
			probnondom *= (1 - dominance_probability_product(_member_struct[m], _member_struct[inextnn->first]) - dominance_probability_product(_member_struct[inextnn->first], _member_struct[m]));
			nn_count++;
		}

		probnondom_map[m] = probnondom;
